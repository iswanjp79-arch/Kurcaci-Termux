#!/bin/bash
# DeepSeek Executor – menggunakan API key dari vault
...
if [ -n "$DEEPSEEK_KEY" ]; then
  curl -s -X POST https://api.deepseek.com/v1/chat/completions \
    ...
    -d "{\"model\":\"deepseek-chat\",\"messages\":[{\"role\":\"user\",\"content\":\"$INPUT\"}],\"max_tokens\":100}" | jq -r '.choices[0].message.content // "Tidak ada respons"'
else
